...
- **Cache optimization**: Post summaries store only essential metadata (title, description, tags, URLs, dates) without full post text. This keeps the cache small (~235 KB for 138 posts) and the worker bundle under Cloudflare's 5MB limit even with 500+ posts. Full text is read directly from MDX during build for feeds.
