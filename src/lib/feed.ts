...
export function buildJsonFeed(locale: Locale) {
  const { feedJsonUrl, homePageUrl, language, title } = getFeedMetadata(locale);
...
export function buildRssFeed(locale: Locale) {
  const { homePageUrl, language, title } = getFeedMetadata(locale);
  const items = mapPostToFeedItem(locale);
  ...
  const rssItems = items
    .map((item) => {
      ...
      const excerptSource = item.content_text ?? "";
      const excerpt = excerptSource.slice(0, 200).trim();
      const descriptionCdata = escapeCdata(excerpt);
